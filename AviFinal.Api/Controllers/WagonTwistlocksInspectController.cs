...
namespace AviAppFinal.Server.Controllers
{
    ...
    [ApiController]
    public class WagonTwistlocksInspectController : ControllerBase
    {
        private readonly AviDbContext _context;
        private readonly IWebHostEnvironment _env;
        private readonly ILogger<WagonTwistlocksInspectController> _logger;

        public WagonTwistlocksInspectController(AviDbContext context, IWebHostEnvironment env, ILogger<WagonTwistlocksInspectController> logger)
        {
            _context = context;
            _env = env;
            _logger = logger;
        ...
        public async Task<IActionResult> UploadPhoto([FromForm] IFormFile file, [FromForm] string formId, [FromForm] string partType, [FromForm] string photoType, [FromForm] string wagonNumber, [FromForm] string wagonGroup)
        {
            ...
            try
            {
                string baseFolder = Path.Combine(_env.WebRootPath, "Twistlocks", formId.ToUpper());
                ...
                string relativePath = Path.Combine("Twistlocks", formId.ToUpper(), subFolder, fileName).Replace("\\", "/");
        ...
        public class WagonTwistlocksInspectDto
        {
        ...
        public async Task<IActionResult> SubmitInspection([FromBody] List<WagonTwistlocksInspectDto> dtos)
        {
            ...
            try
            {
                ...
                var existingRecords = await _context.TwistlocksInspects
                ...
                foreach (var d in dtos)
                {
                    ...
                    if (existingDict.TryGetValue(key, out var existing))
                    {
                        ...
                        existing.TwistlockQty = d.TwistlockQty;
                        existing.LaborValue = d.LaborValue;
                        existing.Phase = d.Phase;
                    }
                    else
                    {
                        await _context.TwistlocksInspects.AddAsync(new TwistlocksInspect
                        {
