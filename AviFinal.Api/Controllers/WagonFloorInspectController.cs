    ...
    public class WagonFloorInspectController : ControllerBase
    {
        ...
        public async Task<IActionResult> SubmitInspection([FromBody] List<WagonFloorInspectDto> dtos)
        {
            ...
            try
            {
                ...
                // (Luca) Add
                var wagonDoors = await _context.WagonGroups
               .Where(w => w.Group == group)
               .Select(w => w.Doors)
               .FirstOrDefaultAsync();

                var wagonTwist = await _context.WagonGroups
                .Where(w => w.Group == group)
                .Select(w => w.Twistlocks)
                .FirstOrDefaultAsync();

                var wagonStan = await _context.WagonGroups
                .Where(w => w.Group == group)
                .Select(w => w.Stanchions)
                .FirstOrDefaultAsync();

                return Ok(new { wagonDoors, wagonTwist, wagonStan });
            }
            catch (Exception ex)
            {
