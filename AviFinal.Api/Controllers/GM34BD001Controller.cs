...
namespace AviFinal.Api.Controllers
{
    ...
    [ApiController]
    public class GM34BD001Controller : ControllerBase
    {
        private readonly AviDbContext _context;
        private readonly IWebHostEnvironment _env;
        private readonly ILogger<GM34BD001Controller> _logger;

        public GM34BD001Controller(AviDbContext context, IWebHostEnvironment env, ILogger<GM34BD001Controller> logger)
        {
            _context = context;
            _env = env;
            _logger = logger;
        ...
        public async Task<IActionResult> UploadPhoto([FromForm] IFormFile file, [FromForm] string formId, [FromForm] string partId, [FromForm] string photoType, [FromForm] string locoNumber, [FromForm] string locoModel)
        {
            ...
            try
            {
                // Base folder: wwwroot/GM34/BD001
                string baseFolder = Path.Combine(_env.WebRootPath, "GM34", formId.ToUpper());

                // Determine subfolder based on photoType
                ...
                string relativePath = Path.Combine("GM34", formId.ToUpper(), subFolder, fileName).Replace("\\", "/");
        ...
        public IActionResult DeletePhoto([FromBody] DeletePhotoRequest4 request)
        {
        ...
        public async Task<IActionResult> SubmitInspection([FromBody] List<GM34BDInspectDto> dtos)
        {
            ...
            try
            {
                var entities = dtos.Select(d => new Gm34bdinspect
                {
                ...
                // Bulk insert
                await _context.Gm34bdinspects.AddRangeAsync(entities);
    ...
    public class DeletePhotoRequest4
    {
    public string Path { get; set; } = string.Empty;
    }
    
    public class GM34BDInspectDto
    {
