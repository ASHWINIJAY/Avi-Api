...
namespace AviAppFinal.Server.Controllers
{
    ...
    [ApiController]
    public class WagonBottomDischargeInspectController : ControllerBase
    {
        private readonly AviDbContext _context;
        private readonly IWebHostEnvironment _env;
        private readonly ILogger<WagonBottomDischargeInspectController> _logger;

        public WagonBottomDischargeInspectController(AviDbContext context, IWebHostEnvironment env, ILogger<WagonBottomDischargeInspectController> logger)
        {
            _context = context;
            _env = env;
            _logger = logger;
        ...
        public async Task<IActionResult> UploadPhoto([FromForm] IFormFile file, [FromForm] string formId, [FromForm] string partType, [FromForm] string photoType, [FromForm] string wagonNumber, [FromForm] string wagonGroup)
        {
            ...
            try
            {
                string baseFolder = Path.Combine(_env.WebRootPath, "BottomDischarge", formId.ToUpper());
                ...
                string relativePath = Path.Combine("BottomDischarge", formId.ToUpper(), subFolder, fileName).Replace("\\", "/");
        ...
        public class WagonBottomDischargeInspectDto
        {
        ...
        public async Task<IActionResult> SubmitInspection([FromBody] List<WagonBottomDischargeInspectDto> dtos)
        {
            ...
            try
            {
                ...
                var existingRecords = await _context.BottomDischargeInspects
                ...
                foreach (var d in dtos)
                {
                    ...
                    if (existingDict.TryGetValue(key, out var existing))
                    {
                        ...
                        existing.MechanismQty = d.MechanismQty;
                        existing.LaborValue = d.LaborValue;
                        existing.Phase = d.Phase;
                    }
                    else
                    {
                        await _context.BottomDischargeInspects.AddAsync(new BottomDischargeInspect
                        {
