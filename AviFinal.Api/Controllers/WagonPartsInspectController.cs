...
namespace AviAppFinal.Server.Controllers
{
    ...
    public class WagonPartsInspectController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetParts(string formID)
        {
            ...
            try
            {
                var partsList = await _context.WagonFinalParts
        ...
        public async Task<IActionResult> GetPartCost(string partId, string field)
        {
            ...
            try
            {
                var part = await _context.WagonFinalParts
        ...
        public async Task<IActionResult> UploadPhoto([FromForm] IFormFile file, [FromForm] string formId, [FromForm] string partId, [FromForm] string photoType, [FromForm] string wagonNumber, [FromForm] string wagonGroup)
        {
            ...
            try
            {
                string baseFolder = Path.Combine(_env.WebRootPath, "WGP", formId.ToUpper());
                ...
                string relativePath = Path.Combine("WGP", formId.ToUpper(), subFolder, fileName).Replace("\\", "/");
        ...
        public class WagonPartsInspectDto
        {
        ...
        public async Task<IActionResult> SubmitInspection([FromBody] List<WagonPartsInspectDto> dtos)
        {
            ...
            try
            {
                ...
                var existingRecords = await _context.WagonPartsInspects
                    .Where(x => wagonNumbers.Contains(x.WagonNumber))
                    .ToListAsync();

                var existingDict = existingRecords.ToDictionary(
                    x => $"{x.WagonNumber}_{x.PartId}"
                );

                foreach (var d in dtos)
                {
                    var key = $"{d.WagonNumber}_{d.PartId ?? ""}";
                    ...
                    {
                        await _context.WagonPartsInspects.AddAsync(new WagonPartsInspect
                        {
